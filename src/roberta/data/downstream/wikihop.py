...
class WikiHopCollator:
    ...
    def process_one_instance(self, instance):
        ...
        shuffle_supports = self.shuffle_supports
        ...
        query_tokens = [q_start] + tokenizer.encode(instance['query'])[1:-1] + [q_end]
        supports_tokens = [tokenizer.encode(support) for support in instance['supports']]
        candidate_tokens = [
        ...
        sort_order = list(range(len(supports_tokens)))
        if shuffle_supports:
            random.shuffle(sort_order)
        for k in sort_order:
            sequence.extend(supports_tokens[k])
            segment_ids.extend([curr_segment_id] * len(supports_tokens[k]))
            curr_segment_id += 1
            if len(sequence) >= self.max_sequence_length:
                break

        assert curr_segment_id <= self.max_num_candidates
