...
class SparseLinear(nn.Linear):
     def __init__(self, in_dim, out_dim):
        ...
        self.tunable_weight_sparsity_mask = nn.Parameter(torch.ones(out_dim, in_dim) * 3)
        self.tunable_bias_sparsity_mask = nn.Parameter(torch.ones(out_dim) * 3)

     def forward(self, inps):
        weight_mask = torch.sigmoid(self.tunable_weight_sparsity_mask)
        bias_mask = torch.sigmoid(self.tunable_bias_sparsity_mask)
        return F.linear(inps, self.weight * weight_mask, self.bias * bias_mask)
...
class RobertaEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.tunable_token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)
        self.tunable_cls_token_embedding = nn.Parameter(0.02 * torch.randn(config.hidden_size))
    ...
    def forward(self, input_ids, token_type_ids, position_ids):
        ...
        token_type_embeddings = self.tunable_token_type_embeddings(token_type_ids)
        position_embeddings = self.position_embeddings(self.pad_token_id + 1 + position_ids)

        embeddings = inputs_embeds + token_type_embeddings + position_embeddings

        embeddings[:, 0, :] = embeddings[:, 0, :] + self.tunable_cls_token_embedding
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = SparseLinear(config.hidden_size, self.all_head_size)
        self.key = SparseLinear(config.hidden_size, self.all_head_size)
        self.value = SparseLinear(config.hidden_size, self.all_head_size)
