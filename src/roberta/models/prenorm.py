...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, "checkpoint_attention"):
            if isinstance(config.checkpoint_attention, list):
                if len(config.checkpoint_attention) > 0:
                    self.checkpoint_attention = config.checkpoint_attention.pop(0)
            else:
    ...
    def extra_repr(self):
        return f"checkpoint_attention={self.checkpoint_attention}"
