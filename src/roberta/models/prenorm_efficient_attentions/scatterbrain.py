...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.attn = SBLocalAttention(local_context = config.attention_window_size, dim_heads = self.attention_head_size)
    ...
    def forward(self, X, mask):
        ...
        attn_out = self.attn(Q.float(), K.float(), V.float(), key_padding_mask = mask).to(X.dtype)
